...
#! Para execução e testes/ criação do executavel basta desativar seu python padrão no sistema caso os seguintes passos não funcionem:
#? 1° baixar o python padrão: | - pip install pyinstaller
#? 2° comando de criação do executavel: | pyinstaller --onefile --name "Sistema_Protocolos" "projeto_integrador (1).py"
#! Lembre de usar o powerShell para instalar direto na maquina sem erros
